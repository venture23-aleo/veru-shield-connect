...
import { PoolBanners } from "./ui/Chrome.js";
...
import { WorkspaceShell, type AppRoute } from "./ui/WorkspaceShell.js";
...
export function App() {
  ...
  const activeGroup = activeLabel?.startsWith("#") ? store.groupByName(activeLabel.slice(1)) : undefined;
  const activeContact = !activeGroup ? (store.contacts.find((c) => c.label === activeLabel) ?? store.contacts[0] ?? null) : null;
    ...
    >
      {route === "tradeoffs" ? (
        ...
        <div className="flex min-h-0 flex-1">
          <div className={`h-full w-full shrink-0 md:block md:w-auto ${mobileThreadOpen ? "hidden" : "block"}`}>
            <Sidebar active={activeId} onSelect={selectConversation} />
          </div>
          <main className={`min-w-0 flex-1 ${mobileThreadOpen || !hasThread ? "flex" : "hidden md:flex"}`}>
            {activeGroup ? (
              <GroupView group={activeGroup} onBack={backToList} />
            ) : activeContact ? (
              <ThreadView contact={activeContact} onBack={backToList} onPrivacy={() => setRoute("tradeoffs")} />
            ) : (
              <div className="m-auto max-w-sm px-6 text-center text-muted-foreground">
                ...
                <p className="mt-2 text-sm">Add a contact by address or invite, or create a group. Messages are end-to-end encrypted and permanent.</p>
              </div>
            )}
          </main>
        </div>
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto bg-surface px-6 py-8 lg:px-10 lg:py-10">
      <div className="w-full rounded-xl bg-surface-lowest p-8 shadow-2xl lg:p-10">
        ...
        <span className="label-caps text-secondary">Compliance model</span>
        ...
        <p className="mt-4 max-w-4xl text-sm leading-relaxed text-muted-foreground lg:text-base">
          STRK20 escrows every user's private viewing key, encrypted to an auditor's public key, on chain at registration.
          An auditor holding lawful process can recover that key and read your history. This is not built for anonymous
          tips or dissident communication.
        </p>
        <p className="mt-3 rounded-lg border border-warn/40 bg-warn-bg px-3 py-2 text-sm text-warn">
          Messages are permanent write-once payloads. Observers see the pool, the helper, the payload size and the timing —
          not the content, and (after the first message to a peer) not the recipient.
        </p>
        <button type="button" className="mt-6 rounded-lg bg-primary px-4 py-2.5 font-semibold text-on-primary" onClick={onTradeoffs}>
          See what this connection keeps private
        </button>
      </div>
    </div>
  );
